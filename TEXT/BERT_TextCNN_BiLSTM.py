...
class BERT_TextCNN_BiLSTM(nn.Module):
    ...
    def forward(self, input, mask, hidden):
        ...
        bert_output = self.bert(input, mask)[0].unsqueeze(1)   # 得到词向量
        attention_weights = F.softmax(torch.sum(bert_output, dim=2), dim=1).unsqueeze(2)  # 注意力权重的维度扩展
        weighted_output = bert_output * attention_weights

        bert_bilstm_out = self.bert(input, mask)[0]

            # bert_output = self.bert(input, mask)[0].unsqueeze(1)
            # # Apply attention mechanism
            # attention_weights = F.softmax(torch.sum(bert_output, dim=2), dim=2).unsqueeze(3)
            # weighted_output = bert_output * attention_weights
            # cnn_out1 = self.conv_and_pool(self.conv1, weighted_output)
            # cnn_out2 = self.conv_and_pool(self.conv2, weighted_output)
            # cnn_out3 = self.conv_and_pool(self.conv3, weighted_output)
