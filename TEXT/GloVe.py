...
def train_predict_svm(user):
    ...
    # 将分词结果转换为对应的 GloVe 向量表示
    vectors = []
    for comment in df['comment']:
        vec = np.zeros(len(list(glove_model.values())[0]))  # 初始化评论向量
        count = 0  # 统计词语数量
        for word in comment:
            if word in glove_model:
...
def train_predict_nb(user):
    ...
    # 对GloVe词嵌入模型进行预处理，确保它们是非负的
    X_train = np.abs(X_train)

    # 构建Multinomial Naive Bayes模型
    model = MultinomialNB()
    model.fit(X_train, y_train)
    ...
    # 对测试数据的GloVe词嵌入模型进行预处理，确保它们是非负的
