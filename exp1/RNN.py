...
class SimpleRNNModel(nn.Module):
    def __init__(self, embedding_matrix, hidden_size, num_classes):
        ...
        self.rnn = nn.RNN(embedding_matrix.embedding_dim, hidden_size, batch_first=True)
...
if __name__ == "__main__":
    ...
    train_dataset = TextDatasetRNN(train_texts, train_labels, word_to_index, MAX_SEQUENCE_LENGTH)
    train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True)

    test_dataset = TextDatasetRNN(test_texts, test_labels, word_to_index, MAX_SEQUENCE_LENGTH)
    test_loader = DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=False)

    model = SimpleRNNModel(embedding_matrix, HIDDEN_SIZE, NUM_CLASSES)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=0.001)

    train_and_evaluate(train_loader, test_loader, model, criterion, optimizer, NUM_EPOCHS)
