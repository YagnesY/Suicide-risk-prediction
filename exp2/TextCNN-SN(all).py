...
def pagerank_with_suicide(G, alpha=0.85, personalization=None,
    ...
    for _ in range(max_iter):
        ...
        for n in x:
            for nbr in W[n]:
                x[nbr] += alpha * xlast[n] * W[n][nbr][weight] * ((1 - a) * (
                        G.nodes[nbr]['activity'] + G.nodes[nbr]['night'] + G.nodes[nbr]['confidence']) + a *
                                                                  G.nodes[nbr]['trisk'])

            x[n] += danglesum * dangling_weights[n] + (1.0 - alpha) * p[n] * (
                        (1 - a) * (G.nodes[n]['activity'] + G.nodes[n]['night'] + G.nodes[n]['confidence']) + a *
                        G.nodes[n]['trisk'])
